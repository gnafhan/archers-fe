...
import { isMobile, openMediaDevices } from '@/utils'
...
const socket = io('http://202.10.36.9:8000')
...
const MeetingRoom = () => {
    ...
    const createNewPeerConnection = async (
        ...
        pc.ontrack = (e) => {
            ...
            if (e.track.kind === 'video') {
                ...
                videoElement.className = 'rounded-lg object-cover bg-cyan-100'
                videoElement.style.maxHeight = '45vh'
    ...
    const handleShare = () => {
        ...
        alert('url tersalin di clipboard')
    }

    const videoElement = (
        <video
            ref={videoRef}
            className='bg-black hidden rounded-md h-[10vh]'
    ...
    useEffect(()=>{
        console.log("upwpwp",videoStream)
    }, [videoStream])
    ...
    return (
        <div className='flex justify-center items-center min-h-screen p-4 bg-slate-800 text-white'>
            {!joined ? (
                ...
                <div className='w-full  h-[100vh]'>
                    <div
                        ...
                        id='video-container'
                        className={`grid grid-cols-1 h-[100vh] sm:grid-cols-2 gap-x-4  justify-center items-center`}
                    >
                        {/* {videoElement} */}
            ...
            <button onClick={handleShare} className="fixed right-5 bottom-5 px-8 py-4 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold rounded-full transition-transform transform-gpu hover:-translate-y-1 hover:shadow-lg">
                Share Link
            </button>
        </div>
    )
